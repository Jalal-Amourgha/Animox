...
const MangaDetails = ({ data, stats, userId }: MangaDetailsProps) => {
  const [openReviewModal, setOpenReviewModal] = useState(false);
  const [openWatchlistModal, setOpenWatchlistModal] = useState(false);
  let reverseArr = stats.scores.reverse();
  const mangaDetails = [
    "type",
    "chapters",
    "volumes",
      ...
      <section className="container mt-[100px] ">
        <div className="bg-bg-color-2 text-white rounded-lg p-3 lg:p-5">
          ...
          <div className="my-5">
            ...
            <div>
              {reverseArr.map((score: any, index: number) => (
                <div className="flex items-center" key={index}>
                  <div className="flex items-center gap-2 text-primary2 text-xl w-20">
                    <FaStar /> {score.score}
                  </div>
                  <div className="max-w-[400px] w-[400px] h-2 bg-white rounded-full overflow-hidden">
                    <div
                      className="h-full bg-primary2"
                      style={{ width: `${score.percentage.toFixed()}%` }}
                    ></div>{" "}
                  </div>
                  <span className="text-primary2 ml-2">
                    {score.percentage}% {"  "}({score.votes} votes)
                  </span>
                </div>
              ))}
            </div>
